...
function GridView() {
    ...
    this.pageItem2 = ko.pureComputed(function () {
        ...
        if (end < count) {
            ...
            //后 -1 0 1 2 3
        else if (pageIndex > (totalPage - count - 1)) {
            for (i = totalPage - count; i <= totalPage; i++) {
                if (i == 0) {
                    continue;
                }
                result.push(i);
    ...
    this.containLastPageItem = function () {
        var result = false;
        for (var pIdx in this.pageItem2()) {

            var curPageIdx = this.pageItem2()[pIdx];

            if (curPageIdx == this.totalPage()) {
                result = true;
                break;
            }
            //if (curPageIdx == (this.totalPage()-1)) {
            //    result = true;
            //    break;
            //}
        }
    ...
    this.showLastPageDot = function () {
        ...
        var result = false;
        var totalPage = this.totalPage();
        var start = this.filter.PageIndex() + 1;
        if (start == totalPage) {
            return false;
        }
    ...
    this.containFirstPageItem = function () {
